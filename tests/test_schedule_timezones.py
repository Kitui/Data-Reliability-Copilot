...
def test_daily_schedule_converts_local_wall_clock_to_utc():
    ...
    after = datetime(2026, 7, 23, 18, 20, tzinfo=UTC)
    result = next_occurrence("daily", 21, 28, None, None, after, -180)
    assert result == datetime(2026, 7, 23, 18, 28, tzinfo=UTC)


def test_daily_schedule_rolls_to_next_local_day_after_due_time():
    after = datetime(2026, 7, 23, 18, 29, tzinfo=UTC)
    result = next_occurrence("daily", 21, 28, None, None, after, -180)
    assert result == datetime(2026, 7, 24, 18, 28, tzinfo=UTC)
