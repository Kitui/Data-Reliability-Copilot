...
def login(client):
    client.post(
        "/auth/register",
        json={
            "full_name": "Copilot Owner",
            "email": "copilot@example.com",
            "organization_name": "Copilot Org",
            "workspace_name": "Copilot Workspace",
...
def test_copilot_context_sessions_and_action_points():
    with TestClient(create_app()) as client:
        ...
        answer = client.post(
            f"/copilot/sessions/{session['id']}/ask",
            json={"question": "Why did the reliability score change?", "analysis_mode": "score"},
        )
        assert answer.status_code == 200
        assert "response" in answer.json()
        point = client.post(
            "/copilot/action-points",
            json={"title": "Review reliability risks", "description": "Review evidence", "session_id": session["id"]},
        )
...
def test_copilot_greeting_is_conversational():
    with TestClient(create_app()) as client:
        ...
        session = client.post("/copilot/sessions", json={"analysis_mode": "general"}).json()
        answer = client.post(
            f"/copilot/sessions/{session['id']}/ask", json={"question": "Hello", "analysis_mode": "general"}
        )
        assert answer.status_code == 200
...
def test_copilot_empty_context_does_not_invent_audit_metrics():
    with TestClient(create_app()) as client:
        ...
        answer = client.post(
            f"/copilot/sessions/{session['id']}/ask",
            json={"question": "Explain my latest audit", "analysis_mode": "general"},
        )
        assert answer.status_code == 200
        response = answer.json()["response"]
