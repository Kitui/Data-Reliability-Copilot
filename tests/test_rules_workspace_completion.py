...
def create_rule(client: TestClient, name: str = 'Required customer email') -> int:
    response = client.post('/quality-rules', json={
        'name': name, 'description': 'Email is required', 'rule_type': 'required',
        'scope': 'column', 'column_name': 'email', 'category': 'completeness',
        'severity': 'high', 'parameters': {}, 'is_active': True,
    })
...
def test_contract_generation_versioning_and_validation():
    with TestClient(create_app()) as client:
        ...
        updated = client.patch(f"/quality-rules/contracts/{contract['id']}", json={
            'dataset_id': dataset['id'], 'name': contract['name'], 'description': 'Second version',
            'status': 'published', 'contract': contract['contract'],
        })
...
def test_bulk_assignments_and_execution_history_export():
    with TestClient(create_app()) as client:
        ...
        bulk = client.post('/quality-rules/assignments/bulk', json={
            'rule_ids': [first, second], 'dataset_ids': [dataset['id']], 'action': 'assign'
        })
...
def test_contract_validation_uses_latest_version_audit_and_assigned_rule_results():
    with TestClient(create_app()) as client:
        ...
        rules = [
            {
                'name': 'Approved account status', 'rule_type': 'allowed_values',
                'scope': 'column', 'column_name': 'account_status', 'category': 'validity',
                'severity': 'high', 'parameters': {'values': ['active', 'inactive', 'suspended']},
                'is_active': True,
            },
            {
                'name': 'Non-negative lifetime value', 'rule_type': 'numeric_range',
                'scope': 'column', 'column_name': 'lifetime_value_kes', 'category': 'validity',
                'severity': 'high', 'parameters': {'min': 0, 'max': None},
                'is_active': True,
            },
        ]
        rule_ids = []
        ...
        assigned = client.post('/quality-rules/assignments/bulk', json={
            'rule_ids': rule_ids, 'dataset_ids': [dataset['id']], 'action': 'assign',
        })
        ...
        assert {item['rule_type'] for item in body['validation']['violations']} == {
            'allowed_values', 'numeric_range'
        }
