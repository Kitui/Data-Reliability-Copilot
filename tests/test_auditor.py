...
def test_analyst_gives_distinct_answers_for_score_and_report_questions() -> None:
    ...
    assert f"move from {result.score.overall}/100 toward 100" in improve_answer.answer
...
def test_contract_generation_prioritizes_assigned_rules_over_observed_bad_values() -> None:
    frame = read_csv_path(Path("samples/customers_dirty.csv"))
    result = audit_dataframe(frame, "customers_dirty.csv")
    rules = [
    ...
    contract = generate_contract(result, rules)

    assert contract.allowed_values["status"] == ["Active", "Inactive"]
    assert contract.numeric_ranges["monthly_spend"].min == 0
    assert contract.numeric_ranges["monthly_spend"].max is None
    assert contract.required_columns == ["customer_id"]
    assert contract.format_rules["email"] == {"format": "email"}
    assert "observed_numeric_ranges" in contract.profile_summary
    assert len(contract.assigned_rule_sources) == 4
