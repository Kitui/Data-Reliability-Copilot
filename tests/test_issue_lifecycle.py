...
def test_issue_lifecycle_updates_comment_and_history():
    with TestClient(create_app()) as client:
        ...
        update = client.patch(
            ...
            json={
                'status': 'in_progress', 'severity': 'high', 'owner': 'Data Steward',
                'due_date': '2026-08-01', 'resolution_note': 'Investigating source-system mapping.',
...
def test_resolving_issue_requires_note_and_improves_score():
    with TestClient(create_app()) as client:
        ...
        rejected = client.patch(
            f"/audits/{audit['audit_id']}/issues/{issue['id']}", json={'status': 'resolved'}
        )
