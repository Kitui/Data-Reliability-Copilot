...
def test_dataset_registry_crud_and_summary():
    with TestClient(create_app()) as client:
        ...
        created = client.post('/datasets', json={
            'name': 'orders_master.csv', 'domain': 'Sales', 'owner_name': 'Data Team',
            'environment': 'production', 'source_type': 'CSV', 'labels': ['Sales', 'Critical']
        })
...
def test_dataset_preview_returns_schema_and_sample_rows():
    with TestClient(create_app()) as client:
        ...
        uploaded = client.post("/audits/upload", files={"file": ("preview.csv", sample, "text/csv")}, data={"rules_json": ""})
...
def test_upload_rejects_duplicate_column_names():
    with TestClient(create_app()) as client:
        login(client)
        response = client.post("/audits/upload", files={"file": ("duplicate.csv", b"id,id\n1,2\n", "text/csv")}, data={"rules_json": ""})
...
def test_column_intelligence_exposes_risk_and_statistics():
    with TestClient(create_app()) as client:
        ...
        uploaded = client.post("/audits/upload", files={"file": ("intelligence.csv", sample, "text/csv")}, data={"rules_json": ""})
        assert uploaded.status_code == 200
...
def test_dataset_metadata_and_labels_persist():
    with TestClient(create_app()) as client:
        ...
        assert created.status_code == 201
        dataset_id = created.json()['id']
        updated = client.patch(f'/datasets/{dataset_id}', json={
...
def test_import_new_dataset_version_creates_audit_lineage_and_schema_drift():
    with TestClient(create_app()) as client:
        ...
        uploaded = client.post('/audits/upload', files={'file': ('customers.csv', first, 'text/csv')}, data={'rules_json': ''})
        assert uploaded.status_code == 200
...
def test_dataset_version_import_is_workspace_scoped():
    with TestClient(create_app()) as client:
        login(client)
        uploaded = client.post('/audits/upload', files={'file': ('scoped.csv', b'id\n1\n', 'text/csv')}, data={'rules_json': ''})
        assert uploaded.status_code == 200
...
def test_version_import_returns_verified_automatic_audit_result():
    with TestClient(create_app()) as client:
        login(client)
        uploaded = client.post('/audits/upload', files={'file': ('verified.csv', b'id,value\n1,a\n', 'text/csv')}, data={'rules_json': ''})
        assert uploaded.status_code == 200
