...
def record_activity(
    *, audit_id: str, issue_id: str, workspace_id: int, actor_user_id: int | None,
    actor_name: str | None, action: str, field_name: str | None = None,
    previous_value: Any = None, new_value: Any = None, note: str | None = None,
    ...
    with session_scope() as db:
        ...
        return IssueLifecycleActivity.model_validate({
            "id": row.id, "audit_id": row.audit_id, "issue_id": row.issue_id,
            "action": row.action, "field_name": row.field_name,
            "previous_value": row.previous_value, "new_value": row.new_value,
            "note": row.note, "actor_user_id": row.actor_user_id,
...
def list_activities(audit_id: str, issue_id: str, workspace_id: int) -> list[IssueLifecycleActivity]:
    with session_scope() as db:
        rows = db.scalars(
            ...
            .where(IssueActivityRecord.audit_id == audit_id,
