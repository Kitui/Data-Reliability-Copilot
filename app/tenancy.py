...
def ensure_bootstrap_tenant() -> None:
    ...
    with session_scope() as db:
        ...
        if not org:
            ...
            db.add(org); db.flush()
        membership = db.scalar(select(OrganizationMembershipRecord).where(OrganizationMembershipRecord.organization_id == org.id, OrganizationMembershipRecord.user_id == user.id))
        if not membership:
            db.add(OrganizationMembershipRecord(organization_id=org.id, user_id=user.id, role="owner", created_at=utcnow()))
        workspace = db.scalar(select(WorkspaceRecord).where(WorkspaceRecord.organization_id == org.id, WorkspaceRecord.slug == "reliability-operations"))
        if not workspace:
            workspace = WorkspaceRecord(organization_id=org.id, name="Reliability Operations", slug="reliability-operations", description="Primary data reliability workspace", is_active=1, created_at=utcnow())
