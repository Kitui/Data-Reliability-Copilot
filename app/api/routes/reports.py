...
def _scope(db, wid, date_from=None, date_to=None, dataset_id=None):
    ...
    if date_from:
        q = q.where(AuditRecord.created_at >= date_from)
    if date_to:
        q = q.where(AuditRecord.created_at <= date_to)
    if dataset_id:
...
def _charts(db, wid, audits):
    ...
    execs = (
        list(db.scalars(select(RuleExecutionRecord).where(RuleExecutionRecord.audit_id.in_(audit_ids))).all())
        if audit_ids
        else []
    )
    days = {}
    ...
    sev = [
        {"label": s.title(), "count": sum(i.severity == s and i.status not in ("resolved", "accepted") for i in issues)}
    ...
    for i in issues:
        ...
        try:
            ...
            for c in match.get("columns", []) or []:
                columns[c] = columns.get(c, 0) + 1
        except Exception:
            logger.exception("Failed to aggregate report column metrics")
    rule_by_day = {}
    for e in execs:
        key = e.executed_at.strftime("%b %d") if e.executed_at else "Unknown"
        rec = rule_by_day.setdefault(key, [0, 0])
        rec[1] += 1
        rec[0] += e.outcome == "passed"
    pass_rate = [{"label": k, "rate": round(v[0] * 100 / v[1], 1) if v[1] else 0} for k, v in rule_by_day.items()]
    latest = {}
    for a in audits:
        latest[a.dataset_name] = a
    rankings = sorted(
        [{"dataset": k, "score": v.score} for k, v in latest.items()], key=lambda x: x["score"], reverse=True
    )
    remediation = []
    for a in audits:
        try:
            ...
            if before is not None:
                remediation.append({"dataset": a.dataset_name, "before": before, "after": a.score})
        except Exception:
            logger.exception("Failed to calculate remediation report metrics")
    return {
        "reliability": reliability,
        "severity": sev,
        "categories": [
            {"label": k.title(), "count": v} for k, v in sorted(categories.items(), key=lambda x: x[1], reverse=True)
        ],
        "rule_pass_rate": pass_rate,
        "dataset_ranking": rankings[:10],
        "affected_columns": [
            {"column": k, "count": v} for k, v in sorted(columns.items(), key=lambda x: x[1], reverse=True)[:10]
        ],
        "remediation_impact": remediation[:10],
        "score_issue_scatter": [
            {"dataset": a.dataset_name, "score": a.score, "issues": a.issue_count} for a in latest.values()
        ],
    }
...
def _dashboard(db, wid, audits):
    ...
    issue_count = (
        db.scalar(
            select(func.count())
            .select_from(IssueRecord)
            .where(IssueRecord.audit_id.in_(ids), IssueRecord.status.not_in(["resolved", "accepted"]))
        )
        if ids
        else 0
    )
    failed_rules = (
        db.scalar(
            select(func.count())
            .select_from(RuleExecutionRecord)
            .where(RuleExecutionRecord.audit_id.in_(ids), RuleExecutionRecord.outcome == "failed")
        )
        if ids
        else 0
    )
    ...
    contracts = (
        db.scalar(
            select(func.count())
            .select_from(DataContractRecord)
            .where(DataContractRecord.workspace_id == wid, DataContractRecord.validation_status == "failed")
        )
        or 0
    )
    return {
        "score": score,
        "datasets": datasets,
        "active_issues": issue_count or 0,
        "failed_rules": failed_rules or 0,
        "contract_violations": contracts,
        "drift_events": sum(1 for a in audits if a.issue_count > 0) // 2,
    }
...
def _issue_columns(audits, issues):
    ...
    for issue in issues:
        match = next(
            (x for x in audit_map.get(issue.audit_id, {}).get("issues", []) if x.get("issue_id") == issue.issue_id), {}
        )
...
def overview_command_centre(user: dict = Depends(require_user)):
    ...
    cutoff = _utc_naive(now() - timedelta(days=7))
    with Session() as db:
        audits = list(
            db.scalars(
                select(AuditRecord).where(AuditRecord.workspace_id == wid).order_by(AuditRecord.created_at.desc())
            ).all()
        )
        latest_by_dataset = {}
        for audit in audits:
        ...
        datasets = list(
            db.scalars(
                select(DatasetRecord).where(DatasetRecord.workspace_id == wid).order_by(DatasetRecord.updated_at.desc())
            ).all()
        )
        current_issues = (
            list(db.scalars(select(IssueRecord).where(IssueRecord.audit_id.in_(latest_audit_ids))).all())
            if latest_audit_ids
            else []
        )
        ...
        executions = (
            list(
                db.scalars(
                    ...
                    .where(RuleExecutionRecord.audit_id.in_(latest_audit_ids))
        ...
        contract_rows = list(
            db.scalars(
                ...
                .order_by(DataContractRecord.updated_at.desc(), DataContractRecord.version.desc())
            ).all()
        )
        latest_contracts = {}
        ...
        alerts = list(
            db.scalars(
                select(AlertRecord).where(AlertRecord.workspace_id == wid).order_by(AlertRecord.detected_at.desc())
            ).all()
        )
        schedules = list(
            db.scalars(
        ...
        previous_latest = {}
        for a in previous:
            previous_latest.setdefault(a.dataset_name, a)
        previous_score = (
            round(sum(a.score for a in previous_latest.values()) / len(previous_latest)) if previous_latest else None
        )
        score_delta = (score - previous_score) if score is not None and previous_score is not None else None
        severity = {
            name: sum(i.severity == name for i in active_issues) for name in ("critical", "high", "medium", "low")
        }
            ...
            lifecycle.append(
                {
                    ...
                    **{
                        level: sum(i.severity == level for i in bucket_issues)
        ...
        for contract in contracts:
            if contract.validation_status in ("failed", "violation") or contract.status == "violation":
                ...
                try:
                    validation = json.loads(contract.validation_json or "{}")
                except (TypeError, json.JSONDecodeError):
                    validation = {}
                contract_violation_count += int(validation.get("violation_count") or 0)

        active_drift_alerts = [
            a for a in alerts if a.alert_type == "schema_drift" and a.status not in ("resolved", "dismissed")
        ]
        ...
        avg_duration = (
            round(sum((r.duration_ms or 0) for r in completed_runs) / len(completed_runs) / 1000, 1)
            if completed_runs
            else 0
        )
        scheduled_success_rate = (
            round(len(completed_runs) * 100 / max(len(completed_runs) + len(failed_runs), 1))
        ...
        audit_success_rate = (
            round(len(recent_audits) * 100 / max(len(recent_audits) + len(failed_runs), 1))
            if (recent_audits or failed_runs)
            else 0
        )
        ...
        for a in audits[:5]:
            activity.append(
                {
                    "kind": "audit",
                    "title": f"Audit completed: {a.dataset_name}",
                    "time": dt(a.created_at),
                    "status": "success",
                    "reference": a.audit_id,
                }
            )
        for r in executions[:4]:
            activity.append(
                {
                    "kind": "rule",
                    "title": f"Rule evaluation {r.outcome}: {r.message[:70]}",
                    "time": dt(r.executed_at),
                    "status": "success" if r.outcome == "passed" else "warning",
                    "reference": r.audit_id,
                }
            )
        for c in contracts[:3]:
            activity.append(
                {
                    "kind": "contract",
                    "title": f"Contract {c.validation_status}: {c.name}",
                    "time": dt(c.updated_at),
                    "status": "success" if c.validation_status == "passed" else "warning",
                    "reference": c.source_audit_id,
                }
            )
        for d in datasets[:3]:
            activity.append(
                {
                    "kind": "dataset",
                    "title": f"Dataset updated: {d.name}",
                    "time": dt(d.updated_at),
                    "status": "info",
                    "dataset_id": d.id,
                }
            )
        ...
        audits_by_dataset = {}
        for audit in audits:
            audits_by_dataset.setdefault(audit.dataset_name, []).append(audit)
        ...
        return {
            "metrics": {
                ...
                "rule_failure_rate": round(failed_rules * 100 / max(execution_total, 1)),
                "contract_violations": contract_violation_count,
                "contracts_impacted": len(violated_contracts),
                "drift_events": len(drift_datasets),
                "drift_datasets": len(drift_datasets),
                "open_remediations": len(open_actions),
                "overdue_remediations": len(overdue_actions),
            },
            "ribbon": {
                "next_run_at": dt(next_schedule.next_run_at) if next_schedule else None,
            ...
            "upcoming": [
                {
                    ...
                    "next_run_at": dt(s.next_run_at),
                    "dataset_id": s.dataset_id,
                }
                for s in active_schedules[:4]
            ],
            "health": {
                "scheduled_runs": len(completed_runs),
                "audit_success_rate": scheduled_success_rate,
                "avg_audit_duration_seconds": avg_duration,
                "active_alerts": len(recent_alerts),
                "connector_status": "healthy"
                if connectors and all(c.status == "active" for c in connectors)
                else ("not_configured" if not connectors else "attention"),
            },
            "platform": {
                "datasets": len(datasets),
                "versions": version_count,
                "audits_this_week": len(recent_audits),
                ...
                "reports": len(reports),
                "action_points": len(action_points),
            },
            "top_columns": [
                {"name": k, "count": v} for k, v in sorted(columns.items(), key=lambda x: x[1], reverse=True)[:5]
            ],
        }
...
def dashboard(
    ...
    with Session() as db:
        ...
        datasets = list(
            db.scalars(
                select(DatasetRecord).where(DatasetRecord.workspace_id == wid).order_by(DatasetRecord.name)
            ).all()
        )
        reports = list(
            db.scalars(
                select(ReportRecord)
                .where(ReportRecord.workspace_id == wid)
        ...
        schedules = list(
            db.scalars(
                ...
                .where(ReportScheduleRecord.workspace_id == wid)
                .order_by(ReportScheduleRecord.created_at.desc())
                .limit(10)
            ).all()
        )
        return {
            "metrics": _dashboard(db, wid, audits),
            "charts": _charts(db, wid, audits),
...
def create_report(body: ReportCreate, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        db.commit()
        db.refresh(r)
        return {"id": r.id, "name": r.name, "format": r.format, "generated_at": r.generated_at}


@router.post("/schedules")
def create_schedule(body: ScheduleCreate, user: dict = Depends(require_user)):
    delta = {"daily": timedelta(days=1), "weekly": timedelta(days=7), "monthly": timedelta(days=30)}.get(
        body.frequency, timedelta(days=7)
    )
    Session = get_session_factory()
    t = now()
    with Session() as db:
...
def delete_schedule(schedule_id: int, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        s = db.scalar(
            select(ReportScheduleRecord).where(
                ReportScheduleRecord.id == schedule_id, ReportScheduleRecord.workspace_id == user["workspace"]["id"]
            )
        )
        if not s:
...
def _csv_bytes(db, wid):
    ...
    out = io.StringIO()
    w = csv.writer(out)
    w.writerow(["Dataset", "Audit ID", "Score", "Issues", "Risk", "Generated At"])
    for a in audits:
        w.writerow([a.dataset_name, a.audit_id, a.score, a.issue_count, a.risk_level, a.created_at.isoformat()])
...
def _pdf_bytes(title, metrics):
    lines = [
        ...
        "",
        f"Overall reliability score: {metrics.get('score', '—')}",
        f"Datasets monitored: {metrics.get('datasets', 0)}",
        f"Active issues: {metrics.get('active_issues', 0)}",
        f"Failed rules: {metrics.get('failed_rules', 0)}",
    ]
    ...
    with Session() as db:
        if format == "csv":
            return StreamingResponse(
                iter([_csv_bytes(db, wid)]),
                media_type="text/csv",
                headers={"Content-Disposition": "attachment; filename=reliability_report.csv"},
            )
        audits, _ = _scope(db, wid)
        metrics = _dashboard(db, wid, audits)
        return Response(
            _pdf_bytes("Executive Reliability Report", metrics),
