...
def _event(dataset: DatasetRecord, before: AuditRecord, after: AuditRecord, drift_type: str, key: str,
           description: str, affected: list[dict], impact: int, version_from: int, version_to: int) -> dict:
    return {
...
def _pair_events(dataset: DatasetRecord, before: AuditRecord, after: AuditRecord, version_from: int, version_to: int) -> list[dict]:
    ...
    if added:
        ...
        events.append(_event(dataset, before, after, "column_added", ",".join(added),
            f"{len(added)} column{'s were' if len(added) != 1 else ' was'} added in version v{version_to}.",
            [{"name": c, "before": None, "after": new[c].get("inferred_type", "unknown")} for c in added], impact, version_from, version_to))
    if removed:
        impact = min(100, 55 + 15 * len(removed))
        events.append(_event(dataset, before, after, "column_removed", ",".join(removed),
            f"{len(removed)} column{'s were' if len(removed) != 1 else ' was'} removed in version v{version_to}.",
            [{"name": c, "before": old[c].get("inferred_type", "unknown"), "after": None} for c in removed], impact, version_from, version_to))
    for name in sorted(set(old) & set(new)):
        old_type, new_type = old[name].get("inferred_type"), new[name].get("inferred_type")
        if old_type != new_type:
            events.append(_event(dataset, before, after, "type_changed", name,
                f"Column {name} changed type from {old_type or 'unknown'} to {new_type or 'unknown'}.",
                [{"name": name, "before": old_type, "after": new_type}], 78, version_from, version_to))
        ...
        if delta >= 0.10:
            ...
            events.append(_event(dataset, before, after, "nullability_changed", name,
                f"Column {name} missing-value rate changed from {old_missing:.1%} to {new_missing:.1%}.",
                [{"name": name, "before": old_missing, "after": new_missing}], impact, version_from, version_to))
        ...
        if old_unique and new_unique and old_unique > 0:
            ...
            if ratio >= 0.30:
                ...
                events.append(_event(dataset, before, after, "cardinality_shift", name,
                    f"Column {name} cardinality shifted from {old_unique} to {new_unique} distinct values.",
                    [{"name": name, "before": old_unique, "after": new_unique}], impact, version_from, version_to))
    return events
        ...
        for dataset in datasets:
            ...
            for item in dataset_events:
                if item["candidate_version"] == latest_version:
                    ...
                    later = set().union(*(signatures_by_pair.get(v, set()) for v in range(item["candidate_version"] + 1, latest_version + 1)))
...
def list_schema_drift(
    dataset_id: int | None = Query(default=None), drift_type: str = Query(default="all"),
    severity: str = Query(default="all"), status: str = Query(default="all"),
    search: str = Query(default=""), user: dict = Depends(require_user),
):
    events = _all_events(user["workspace"]["id"])
    if dataset_id is not None: events = [x for x in events if x["dataset_id"] == dataset_id]
    if drift_type != "all": events = [x for x in events if x["drift_type"] == drift_type]
    if severity != "all": events = [x for x in events if x["severity"] == severity]
    if status != "all": events = [x for x in events if x["status"] == status]
    if search.strip():
    ...
    return {
        "summary": {"total": len(events), "high": severities["high"], "medium": severities["medium"], "low": severities["low"], "average_impact": average},
...
def export_schema_drift(user: dict = Depends(require_user)):
    ...
    writer.writerow(["Dataset", "Drift type", "Severity", "Impact score", "Status", "Baseline version", "Candidate version", "Detected", "Description"])
    for item in events:
        writer.writerow([item["dataset_name"], item["drift_type"], item["severity"], item["impact_score"], item["status"], item["baseline_version"], item["candidate_version"], item["detected_at"], item["description"]])
    return StreamingResponse(iter([output.getvalue()]), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=schema_drift_report.csv"})
...
def get_schema_drift_event(event_id: str, user: dict = Depends(require_user)):
    ...
    if event is None: raise HTTPException(404, "Drift event not found in the active workspace.")
    return event
