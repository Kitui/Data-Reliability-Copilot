...
def _password_or_400(password: str) -> None:
    ...
    if errors:
        raise HTTPException(
            status_code=400, detail={"message": "Password does not meet security requirements.", "requirements": errors}
        )
...
def _issue_account_token(db, user_id: int, purpose: str, minutes: int) -> str:
    ...
    now = utcnow()
    db.execute(
        delete(AccountTokenRecord).where(
            AccountTokenRecord.user_id == user_id,
            AccountTokenRecord.purpose == purpose,
            AccountTokenRecord.used_at.is_(None),
        )
    )
    db.add(
        AccountTokenRecord(
...
def register(payload: RegistrationRequest, request: Request, response: Response) -> dict[str, object]:
    ...
    with session_scope() as db:
        ...
        organization = OrganizationRecord(
            name=organization_name, slug=_unique_slug(db, OrganizationRecord, organization_name), created_at=utcnow()
        )
        db.add(organization)
        db.flush()
        workspace = WorkspaceRecord(
            ...
            name=workspace_name,
            slug=_unique_slug(db, WorkspaceRecord, workspace_name, organization.id),
        ...
        user = UserRecord(
            ...
            is_active=1,
            created_at=utcnow(),
            email_verified_at=None if settings.require_email_verification else utcnow(),
        )
        db.add(user)
        db.flush()
        db.add(
        ...
        verification_token = (
            _issue_account_token(db, user.id, "email_verification", 1440)
...
def login(payload: LoginRequest, request: Request, response: Response) -> dict[str, object]:
        ...
        recent_failures = (
            db.scalar(
                select(func.count(LoginAttemptRecord.id)).where(
                    ...
                    LoginAttemptRecord.succeeded == 0,
                    LoginAttemptRecord.attempted_at >= window_start,
                )
            )
            or 0
        )
            ...
            if not valid:
                ...
                workspace = (
                    db.scalar(
                        ...
                        .where(
                            WorkspaceRecord.organization_id == membership.organization_id,
                            WorkspaceRecord.is_active == 1,
                        )
    ...
    if failure_status:
        record_security_event(
            ...
            outcome="blocked" if failure_status == 429 else "failed",
            details={"email": email},
        )
...
def revoke_session(session_id: int, request: Request, user: dict[str, object] = Depends(require_user)) -> Response:
    ...
    record_security_event(
        "auth.session_revoke", request=request, user=user, resource_type="session", resource_id=session_id
    )
...
def request_password_reset(payload: TokenRequest, request: Request) -> dict[str, object]:
    ...
    with session_scope() as db:
        ...
        if user and user.is_active:
            raw = _issue_account_token(db, user.id, "password_reset", 30)
    ...
    if raw and settings.expose_dev_tokens and not settings.is_production:
...
def confirm_password_reset(payload: TokenConfirm, request: Request) -> dict[str, str]:
    if not payload.password:
        raise HTTPException(400, "A new password is required.")
    _password_or_400(payload.password)
    now = utcnow()
    with session_scope() as db:
        token = db.scalar(
            select(AccountTokenRecord).where(
                AccountTokenRecord.token_hash == token_digest(payload.token),
                AccountTokenRecord.purpose == "password_reset",
                AccountTokenRecord.used_at.is_(None),
            )
        )
        if not token or token.expires_at <= now:
            raise HTTPException(400, "The reset token is invalid or expired.")
    ...
    record_security_event(
        "auth.password_reset", request=request, actor_user_id=user.id, resource_type="user", resource_id=user.id
    )
...
def request_email_verification(request: Request, user: dict[str, object] = Depends(require_user)) -> dict[str, object]:
    ...
    with session_scope() as db:
        ...
        if row and not row.email_verified_at:
            raw = _issue_account_token(db, row.id, "email_verification", 1440)
...
def confirm_email_verification(payload: TokenConfirm, request: Request) -> dict[str, str]:
    ...
    with session_scope() as db:
        token = db.scalar(
            select(AccountTokenRecord).where(
                AccountTokenRecord.token_hash == token_digest(payload.token),
                AccountTokenRecord.purpose == "email_verification",
                AccountTokenRecord.used_at.is_(None),
            )
        )
        if not token or token.expires_at <= now:
            raise HTTPException(400, "The verification token is invalid or expired.")
        user = db.get(UserRecord, token.user_id)
        user.email_verified_at = now
        token.used_at = now
    record_security_event(
        "auth.email_verified", request=request, actor_user_id=user.id, resource_type="user", resource_id=user.id
    )
