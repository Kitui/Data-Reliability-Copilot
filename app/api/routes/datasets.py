...
from app.ingestion import IngestionError, read_csv_bytes
    ...
    with Session() as db:
        row = db.scalar(
            select(DatasetRecord).where(
                DatasetRecord.workspace_id == workspace_id, DatasetRecord.name == audit.dataset_name
            )
        )
...
@router.get("")
def list_datasets(
    search: str = Query(default=""), environment: str = Query(default="all"), user: dict = Depends(require_user)
):
    ...
    with Session() as db:
        ...
        if search.strip():
            stmt = stmt.where(func.lower(DatasetRecord.name).contains(search.strip().lower()))
        if environment != "all":
            stmt = stmt.where(DatasetRecord.environment == environment)
...
def dataset_summary(user: dict = Depends(require_user)):
    ...
    return {
        "registered": len(rows),
        "monitored": sum(r.latest_audit_id is not None for r in rows),
        "active_issues": sum(r.issue_count > 0 for r in rows),
        "recently_updated": sum(
            (
                datetime.now(UTC) - (r.updated_at.replace(tzinfo=UTC) if r.updated_at.tzinfo is None else r.updated_at)
            ).days
            <= 7
            for r in rows
        ),
...
def create_dataset(payload: DatasetCreate, user: dict = Depends(require_user)):
    ...
    row = DatasetRecord(
        ...
        owner_name=payload.owner_name.strip() or user["full_name"],
        environment=payload.environment,
        status="archived" if payload.environment == "archived" else "registered",
        source_type=payload.source_type.strip() or "CSV",
        description=payload.description,
        labels_json=json.dumps(sorted(set(x.strip() for x in payload.labels if x.strip()))),
        record_count=0,
        column_count=0,
        issue_count=0,
        created_at=now,
...
def get_dataset(dataset_id: int, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        row = db.scalar(
            select(DatasetRecord).where(
                DatasetRecord.id == dataset_id, DatasetRecord.workspace_id == user["workspace"]["id"]
            )
        )
        if row is None:
        ...
        history = db.scalars(
            select(AuditRecord)
            .where(AuditRecord.workspace_id == row.workspace_id, AuditRecord.dataset_name == row.name)
            .order_by(AuditRecord.created_at.asc())
        ).all()
        data = serialize_with_lineage(db, row)
        data["score_history"] = [{"score": a.score, "created_at": a.created_at} for a in history[-14:]]
        return data
...
def preview_dataset(dataset_id: int, limit: int = Query(default=10, ge=1, le=50), user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        if not row.latest_audit_id:
            return {
                "available": False,
                "message": "A preview becomes available after the dataset is imported and audited.",
                "columns": [],
                "rows": [],
            }
        ...
        audit = db.scalar(
            select(AuditRecord).where(
                AuditRecord.audit_id == row.latest_audit_id, AuditRecord.workspace_id == row.workspace_id
            )
        )
        if audit is None:
            return {
                "available": False,
                "message": "No completed audit is available for this dataset.",
                "columns": [],
                "rows": [],
            }
...
def dataset_privacy(dataset_id: int, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        if not row.latest_audit_id:
            return {
                "available": False,
                "message": "Privacy intelligence becomes available after an audit.",
                "findings": [],
            }
        audit = db.scalar(
            select(AuditRecord).where(
...
def update_dataset(dataset_id: int, payload: DatasetUpdate, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        if payload.labels is not None:
            row.labels_json = json.dumps(sorted(set(x.strip() for x in payload.labels if x.strip())))
        row.updated_at = now
        db.commit()
        db.refresh(row)
        return serialize(row)
...
def delete_dataset(dataset_id: int, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        audit_ids = list(
            db.scalars(
                select(AuditRecord.audit_id).where(
        ...
        if audit_ids:
            storage_keys = list(
                db.scalars(
                    select(UploadRecord.relative_path).where(
...
def _save_version_upload(
    content: bytes, original_filename: str, content_type: str | None, workspace_id: int
) -> UploadedFileInfo:
    return build_dataset_file_service().save_upload(
        content, original_filename, content_type, workspace_id, category="versions"
    )
...
def dataset_versions(dataset_id: int, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        return {
            ...
            "versions": [
                _version_payload(audit, index + 1, dataset.latest_audit_id) for index, audit in enumerate(audits)
            ],
        }
