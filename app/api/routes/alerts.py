    ...
    *,
    ...
    if row is None:
        row = AlertRecord(
            ...
            reference_json=json.dumps(reference or {}),
...
def sync_alerts(workspace_id: int, user_id: int) -> None:
    ...
    with Session() as db:
        ...
        datasets = {
            d.id: d for d in db.scalars(select(DatasetRecord).where(DatasetRecord.workspace_id == workspace_id)).all()
        }
        audits = list(
            db.scalars(
                select(AuditRecord)
        ...
        for audit in latest_by_dataset.values():
            ...
            if audit.score < threshold:
                ...
                _upsert(
                    db,
                    workspace_id,
                    fingerprint=f"score:{audit.audit_id}:{threshold}",
                    alert_type="score_threshold",
                    severity=severity,
                    title=f"Reliability score below threshold ({threshold}%)",
                    ...
                    reference={"score": audit.score, "threshold": threshold},
            ...
            if high_issues:
                ...
                _upsert(
                    db,
                    workspace_id,
                    fingerprint=f"issues:{audit.audit_id}",
                    alert_type="high_severity_issue",
                    severity=severity,
                    ...
                    dataset_name=audit.dataset_name,
                    audit_id=audit.audit_id,
                    reference={"issue_count": len(high_issues)},
                    detected_at=audit.created_at,
                )
        failed_execs = list(
            db.scalars(
                ...
                .where(AuditRecord.workspace_id == workspace_id, RuleExecutionRecord.outcome == "failed")
        ...
        for execution in failed_execs:
            ...
            _upsert(
                db,
                workspace_id,
                fingerprint=f"rule:{execution.id}",
                alert_type="rule_failure",
                severity=(rule.severity if rule else "medium"),
                title="Quality rule failed",
                description=execution.message,
                potential_impact="Failed quality rules may invalidate expected business constraints.",
                dataset_id=dataset.id if dataset else None,
                dataset_name=audit.dataset_name if audit else None,
                audit_id=execution.audit_id,
                rule_id=execution.rule_id,
                reference={
                    "rule_name": rule.name if rule else "Quality rule",
        ...
        contracts = list(
            db.scalars(
                ...
                .order_by(DataContractRecord.updated_at.desc())
                .limit(40)
            ).all()
        )
        for contract in contracts:
            ...
            _upsert(
                db,
                workspace_id,
                fingerprint=f"contract:{contract.id}:{contract.version}",
                alert_type="contract_violation",
                severity="high",
                title="Data contract violation",
                ...
                audit_id=contract.source_audit_id,
        ...
        for event in _all_events(workspace_id):
            ...
            _upsert(
                ...
                reference={
                    ...
                    "baseline_version": event.get("baseline_version"),
        ...
        for run in failed_runs:
            ...
            _upsert(
                db,
                workspace_id,
                fingerprint=f"schedule:{run.id}",
                alert_type="scheduled_audit_failure",
                severity="critical",
                ...
                reference={"run_id": run.id, "schedule_id": run.schedule_id},
...
def dashboard(
    ...
    with Session() as db:
        ...
        if search:
            term = f"%{search.strip()}%"
            query = query.where(
                (AlertRecord.title.ilike(term))
                | (AlertRecord.dataset_name.ilike(term))
                | (AlertRecord.description.ilike(term))
            )
        ...
        if dataset_id:
            query = query.where(AlertRecord.dataset_id == dataset_id)
        if date_from:
            query = query.where(AlertRecord.detected_at >= date_from)
        if date_to:
            query = query.where(AlertRecord.detected_at <= date_to)
        ...
        datasets = list(
            db.scalars(
                select(DatasetRecord).where(DatasetRecord.workspace_id == workspace_id).order_by(DatasetRecord.name)
            ).all()
        )
    ...
    return {
        "alerts": [serialize(x) for x in page_rows],
        "metrics": metrics,
...
def update_alert(alert_id: int, payload: AlertAction, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        if payload.action == "read":
            row.read_at = now
            row.status = "read" if row.status == "new" else row.status
...
def get_preferences(user: dict = Depends(require_user)):
    ...
    with Session() as db:
        row = db.scalar(
            select(NotificationPreferenceRecord).where(
                NotificationPreferenceRecord.workspace_id == wid, NotificationPreferenceRecord.user_id == user["id"]
            )
        )
        if not row:
            return PreferencePayload().model_dump()
        return {
            k: bool(getattr(row, k))
            for k in [
                "in_app_enabled",
                "email_enabled",
                "critical_enabled",
...
def save_preferences(payload: PreferencePayload, user: dict = Depends(require_user)):
    ...
    with Session() as db:
        ...
        if not row:
            row = NotificationPreferenceRecord(workspace_id=wid, user_id=user["id"], updated_at=now)
            db.add(row)
...
def export_csv(user: dict = Depends(require_user)):
    ...
    writer.writerow(["Alert", "Type", "Severity", "Status", "Dataset", "Detected at", "Audit ID", "Description"])
    for row in rows:
        writer.writerow(
            [
                row.title,
                ...
                row.status,
                row.dataset_name or "",
                ensure_utc(row.detected_at).isoformat(),
                row.audit_id or "",
                row.description,
            ]
        )
