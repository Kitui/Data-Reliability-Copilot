...
def require_user(drc_session: str | None = Cookie(default=None)) -> dict[str, object]:
    ...
    with session_scope() as db:
        row = db.execute(
            ...
            .where(SessionRecord.token_hash == token_digest(drc_session), SessionRecord.revoked_at.is_(None))
        ).first()
        if not row:
        ...
        if not workspace:
            ...
            if membership:
                workspace = db.scalar(
                    ...
                    .where(
                        WorkspaceRecord.organization_id == membership.organization_id, WorkspaceRecord.is_active == 1
                    )
