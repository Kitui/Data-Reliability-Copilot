...
from app.privacy import classify_sensitive_column, SENSITIVITY_ORDER
...
def profile_dataset(frame: pd.DataFrame) -> DatasetProfile:
    ...
    for column in frame.columns:
        ...
        risk_score, risk_level, signals = risk_assessment(
            missing_rate=missing_rate, outlier_rate=outlier_rate,
            constant=constant, inferred_type=inferred_type,
        )
        ...
        columns.append(ColumnProfile(
            name=name, inferred_type=inferred_type, missing_count=missing_count,
            missing_rate=missing_rate, unique_count=unique_count, unique_rate=unique_rate,
            sample_values=samples, stats=column_stats(series, inferred_type),
            cardinality=cardinality, constant=constant, identifier_candidate=identifier_candidate,
            outlier_count=outlier_count, outlier_rate=outlier_rate,
            risk_score=risk_score, risk_level=risk_level, signals=signals,
    ...
    return DatasetProfile(
        row_count=row_count, column_count=len(frame.columns), duplicate_row_count=duplicate_count,
        ...
        sensitive_column_count=len(sensitive), highest_sensitivity=highest,
        columns=columns,
    )
