...
def _is_boolean_or_email_metadata(column: str, series: pd.Series) -> bool:
    lowered = column.strip().lower().replace(" ", "_")
    metadata_tokens = ("verified", "verification", "valid", "is_", "has_", "consent", "opt_in", "flag", "status")
    if "email" in lowered and any(token in lowered for token in metadata_tokens):
        return True
    if pd.api.types.is_bool_dtype(series):
        return True
    values = {str(value).strip().lower() for value in series.dropna().head(250)}
    boolean_tokens = {"true", "false", "yes", "no", "y", "n", "1", "0"}
    return bool(values) and values.issubset(boolean_tokens)
...
def _detect_email_issues(frame: pd.DataFrame, add_issue) -> None:
    for column in frame.columns:
        if "email" not in str(column).lower():
            continue
        if _is_boolean_or_email_metadata(str(column), frame[column]):
            continue
...
def _detect_pii_columns(frame: pd.DataFrame, add_issue) -> None:
    ...
    add_issue(
        ...
        f"The dataset contains {len(columns)} sensitive column(s), classified as {classifications}. "
        f"Highest sensitivity is {highest['sensitivity']}.",
        columns,
        len(frame),
        "Apply column-specific masking or tokenisation, restrict access, and avoid exposing raw values in reports or AI prompts.",
