...
def summarize_audit(profile: DatasetProfile, issues: list[QualityIssue], score: QualityScore) -> AuditSummary:
    active_issues = [issue for issue in issues if issue.status not in {"fixed", "resolved", "ignored"}]
    context = build_llm_context(profile, active_issues, score)
    fallback = build_rule_based_summary(profile, active_issues, score, context)
